...
def decode_preview(preview, version):
    ...
    match version:
        ...
        case 'sd3' | 'sd3Large':
            ...
            # image = Image.frombytes('RGBA', (width, height), bytes(bytes_array))
            image = None

        case 'sdxlBase' | 'sdxlRefiner' | 'sdxl_base_v0.9' | 'ssd1b' | 'pixart' | 'auraflow':
            # for i in range(height * width):
            ...
            #     bytes_array[i * 4 + 2] = max(min(int(b), 255), 0)
            #     bytes_array[i * 4 + 3] = 255
            # image = Image.frombytes('RGBA', (width, height), bytes_array)
            image = None
